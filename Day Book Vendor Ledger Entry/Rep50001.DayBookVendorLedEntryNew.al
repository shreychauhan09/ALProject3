...
report 50001 "Day Book Vendor Led Entry New"
{
    ...
    dataset
    {
        dataitem(ReqVendLedgEntry; "Vendor Ledger Entry")
        {
            ...
            begin
                //Opening Balance++
                // StartDate := ReqVendLedgEntry.GetRangeMin("Posting Date");
                // vendorrec.SetRange("No.", ReqVendLedgEntry."Vendor No.");
                // vendorrec.SetFilter("Date Filter", '..%1', StartDate);
                // if vendorrec.FindFirst() then
                //     vendorrec.CalcFields("Net Change (LCY)");
                // NetChangeLCY := vendorrec."Net Change (LCY)";
                // Message(vendorrec."No.");
                // Message(Format(vendorrec."Net Change (LCY)"));
                // Message(Format(vendorrec."Date Filter"));
                CurrReport.Break();
                //Opening Balance--
            end;
        }
        dataitem(Date; Date)
        {
            ...
            {
                DataItemTableView = sorting("Document Type", "Vendor No.", "Posting Date", "Currency Code");
                column(VendorOpeningNetLCY; NetChangeLCY)
                {
                }
                column(vendorrec; vendorrec."Net Change (LCY)")
                {
                }
                ...
                //         SETRANGE("Document No.", "Vendor Ledger Entry"."Document No.");
                //         SetRange("Posting Date", "Vendor Ledger Entry"."Posting Date");
                //         SetRange("Entry No.", "Vendor Ledger Entry"."Entry No.");
                //     end;
                // }
                trigger OnAfterGetRecord()
                var
